...
for dgp in [2, 3]:
    ...
    for lev_name, g_lev in level_gen:
        lev_name = str(lev_name)  # ── 保证level是字符串，避免后续拼接/排序混乱
        ...
        # 用“因子 + level + 轴向”构造唯一标识
        stable_cfg_n = f"{FACTOR_NAME}::{lev_name}::n"
        stable_cfg_d = f"{FACTOR_NAME}::{lev_name}::d"

        for m in metrics:
            rows2.append({
                'config_name': stable_cfg_n,
                'dgp_num': dgp,
                'factor': FACTOR_NAME,
                'level': lev_name,
                'axis': 'n_samples',
